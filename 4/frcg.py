...
def frcg(fun, gfun, x0, epsilon=1e-7, maxiter=5000, verb=True):
    n = len(x0)
    g0 = gfun(x0)
    d0 = -g0
    k = 0
...
if __name__ == '__main__':
    ...
    fun = lambda x: 0.5 * (x.T @ A.T @ A @ x + 2 * b.T @ A @ x + b.T @ b) + 0.05 * (x - y).T @ (x - y)
    gfun = lambda x: A.T @ (A @ x + b) + 0.1 * (x - y)

    x0 = np.zeros(n).reshape(n, 1)
    # xk, fk, k = cg(fun, gfun, x0, A)
    # print('--------------------------------------------------')
    xk, fk, k = frcg(fun, gfun, x0, verb=False)
    print_vector(xk, 5)
